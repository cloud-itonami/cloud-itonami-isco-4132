(ns data-entry.advisor
  "DataEntryAdvisor — proposes a data-entry operation (enter a value,
  verify a value, enter data into a regulated financial or medical
  record system, override a validation failure) for a registered
  dataset. The advisor is swappable: `mock-advisor` (deterministic,
  ...
  dataset provenance or regulated-record/validation-override risk;
  `data-entry.governor` is the independent system that decides
  whether the proposal may proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :enter|:verify|:enter-regulated-record|:override-validation-failure
...
(defn- infer
  ...
  {:op op
   ...
   :rationale (str "proposed " (name op) " for dataset " (:dataset-id request))})
...
(def ^:private system-prompt
  "You are a data-entry advisor. Given an entry operation request,
