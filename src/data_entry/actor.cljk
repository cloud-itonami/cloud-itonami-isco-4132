(ns data-entry.actor
  "DataEntryActor — the ISCO-08 4132 independent data-entry actor as a
  `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one entry operation request
  ...
  The unconditional invariant: the DataEntryAdvisor can never directly
  commit a record or dispatch a robot action the DataEntryGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [data-entry.advisor :as advisor]
            [data-entry.governor :as governor]
            [data-entry.store :as store]))

(defn build-graph
  "Build a compiled DataEntryActor graph. `store` implements
  `data-entry.store/Store`. `advisor` implements
  `data-entry.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:dataset-id (:dataset-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
