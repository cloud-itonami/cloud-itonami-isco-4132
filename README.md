...
- `src/data_entry/store.cljk` — `Store` protocol + `MemStore`:
  registered datasets, committed records, an append-only audit ledger.
- `src/data_entry/advisor.cljk` — `Advisor` protocol; `mock-advisor`
...
- `src/data_entry/governor.cljk` — `DataEntryGovernor/check`: a pure
...
- `src/data_entry/actor.cljk` — `build-graph`, `run-request!`,
